...
class ExecutionConfig:
    ...
    submarkets: List[Dict[str, str]] = field(default_factory=list)
    ...
    def __post_init__(self) -> None:
        ...
        self.submarkets = [
            self._normalize_submarket_entry(entry) for entry in self.submarkets or []
        ]

    @staticmethod
    def _normalize_submarket_entry(entry: object) -> Dict[str, str]:
        if isinstance(entry, str):
            entry = {"id": entry.strip(), "name": entry.strip()}
        if isinstance(entry, dict):
            ...
            if market_id is None:
                raise ValueError("Each submarket entry must provide an id/token_id")
            market_id_str = str(market_id).strip()
            if not market_id_str:
                raise ValueError("Submarket id cannot be empty")
            name = entry.get("name") or entry.get("label") or entry.get("title")
            name_str = str(name).strip() if name is not None else market_id_str
            return {"id": market_id_str, "name": name_str}
        raise ValueError("submarkets must be strings or dicts with an id/token_id field")
